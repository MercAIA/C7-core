...
# ================== Phase 6 (CLEAN) — Strict Separation of Core vs Intensity/Phase ==================
...
# ----- Core Brain: کاملاً مستقل از شدت و فاز -----
class C7Brain:
...
# ----- Phase Manager: فقط ناظر شدت، بدون دخالت در مغز -----
class PhaseManager:
    """
    0: Phase A
    1: Boundary A→B
    ...
    def maybe_transition(self, intensity):
        ...
        # فقط اگر در مرز هستیم و شدت از میانگین با ضریب خاصی بالاتره
    ...
    def soft_drift(self, intensity):
        # فقط حرکت آهسته بین فاز A/B و مرزها — باز هم بدون دخالت در مغز
...
class MetaObserver:
    ...
    def observe(self, a1, a3, a5, a7, target, intensity, phase_state, tmem: TemporalMemory):
        ...
        stability_score = float(np.exp(-var_arrays / 1000.0) * np.exp(-abs_error / 10.0))
...
def main():
    ...
    print("Starting Phase 6 (CLEAN) training for 300 steps...\n")
    ...
    for step in range(1, num_steps + 1):
        ...
        # فاز فقط برای گزارش و چرخه است، نه برای آموزش
    ...
    print("\nPhase 6 (CLEAN) training finished.\n")

    # ======= TEST AFTER TRAINING =======
    print("========== TEST AFTER PHASE 6 (CLEAN) ==========\n")

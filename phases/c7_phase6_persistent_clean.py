...
# ============= Phase 6 PERSISTENT (CLEAN) =============
#  - Core brain مستقل از شدت و فاز
#  - Phase / Intensity فقط ناظر
#  - وزن‌ها بین اجراها ذخیره می‌شن

random.seed(42)
np.random.seed(42)

BRAIN_PATH = "c7_brain_phase6_clean.npz"
...
def compute_intensity(a_out, t_out, i_out):
    ...
    energies = {"audio": audio_e, "text": text_e, "image": image_e}
...
class C7Brain:
    ...
    @classmethod
    def load_or_create(cls, path=BRAIN_PATH, input_dim=9):
        if os.path.exists(path):
            data = np.load(path)
            brain = cls(input_dim=input_dim)
            brain.W1 = data["W1"]
            brain.W3 = data["W3"]
            brain.W5 = data["W5"]
    ...
    def save(self, path=BRAIN_PATH):
        np.savez(path, W1=self.W1, W3=self.W3, W5=self.W5)
    ...
    def train_step(self, emb_c, target, lr=0.0003):
...
class PhaseManager:
    ...
    def __init__(self, initial_state=0, intensity_window=20, threshold_factor=0.10):
...
class TemporalMemory:
    ...
    def push(self, a7_output, intensity, phase_state):
        self.history.append(
            {"a7": a7_output, "intensity": intensity, "phase": phase_state}
        )

    def summary(self):
        if not self.history:
            return {"len": 0, "avg_a7": None, "avg_intensity": None}
...
class MetaObserver:
    ...
    def observe(self, a1, a3, a5, a7, target, intensity, phase_state, tmem: TemporalMemory):
        ...
        stability_score = float(
            np.exp(-var_arrays / 1000.0) * np.exp(-abs_error / 10.0)
        )

        tmem_summary = tmem.summary()
        recent_int = (
            tmem_summary["avg_intensity"]
            if tmem_summary["avg_intensity"] is not None
            else 0.0
        )
...
def main():
    brain = C7Brain.load_or_create(input_dim=9)
    phase_manager = PhaseManager()
    tmem = TemporalMemory(maxlen=10)
    meta = MetaObserver()

    print("Starting Phase 6 PERSISTENT (CLEAN) training for 300 steps...\n")
    ...
    for step in range(1, num_steps + 1):
        ...
        if step % 50 == 0:
            ...
            print(
                f"Step {step}/{num_steps} - avg loss: {avg_loss:.4f} "
                f"- phase: {phase_manager.state_name()} - trigger: {trigger}"
            )

    print("\nPhase 6 PERSISTENT (CLEAN) training finished.\n")
    brain.save()

    # ======= TEST AFTER TRAINING =======
    print("========== TEST AFTER PHASE 6 PERSISTENT (CLEAN) ==========\n")
